...
SOLUTION_FILE = 'src/Nancy.Serialization.JsonApi.sln'
...
assemblyinfo :version => [:clean] do |asm|
    ...
    asm.product_name = "Nancy.Serialization.JsonApi"
    asm.title = "Nancy.Serialization.JsonApi"
    asm.description = "Provides JSON (de)serialization support using Newtonsoft.JsonApi."
...
zip :package => [:publish] do |zip|
    ...
    zip.output_file = "Nancy.Serialization.JsonApi-Latest.zip"
...
task :nuget_package => [:publish] do
    ...
    nuspecs = FileList["src/Nancy.Serialization.JsonApi/*.nuspec"]
    root = File.dirname(__FILE__)

    # Copy all project *.nuspec to nuget build folder before editing
    FileUtils.cp_r nuspecs, "#{OUTPUT}/nuget"
    nuspecs = FileList["#{OUTPUT}/nuget/Nancy.Serialization.JsonApi.nuspec"]
    ...
    nuspecs.each do |nuspec|
        update_xml nuspec do |xml|
            ...
            xml.root.elements["metadata/licenseUrl"].text = "https://github.com/NancyFx/Nancy.Serialization.JsonApi/blob/master/license.txt"
